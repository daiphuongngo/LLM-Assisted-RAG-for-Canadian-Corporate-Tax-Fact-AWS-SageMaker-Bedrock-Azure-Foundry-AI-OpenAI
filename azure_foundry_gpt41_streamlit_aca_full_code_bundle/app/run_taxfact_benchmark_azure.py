...
def build_retrieval_rag() -> TaxRAGV52Core:
    ...
    # Safety guard: prevent local Qwen download/load.
    rag.load_llm = lambda *args, **kwargs: (None, None)

    rag.extract_bundle_if_needed()
    rag.load_artifacts()

    # Force again after artifact/run_config loading.
    ...
    print(
        "Retrieval-only flags:",
        f"use_llm_planner={rag.settings.use_llm_planner}",
        f"use_llm_answer={rag.settings.use_llm_answer}",
        f"use_llm_verifier={rag.settings.use_llm_verifier}",
    )
    print("Local Qwen loading disabled. S3/AWS disabled. Azure OpenAI is used only for final answer generation.")
...
def run_one(rag: TaxRAGV52Core, backend: str, qmeta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if backend == "no_llm":
        ...
        t1 = time.time()
        model_result = answer_with_azure_openai(qmeta["question"], qmeta, evidence)
...
def write_outputs(output: Dict[str, Any], output_dir: Path, label: str) -> None:
    ...
    with txt_path.open("w", encoding="utf-8") as f:
        ...
        for r in output["rows"]:
            ...
            f.write(f"Answer:\n{r['answer']}\n\n")
            f.write(
                f"Scores: page_hit={r['page_hit']} "
                f"exact={r['exact_match']} "
                f"contains={r['contains_expected']} "
                f"value_hit={r['value_hit']}\n"
            )
...
def main() -> None:
    ...
    ap.add_argument("--backend", required=True, choices=["no_llm", "azure_openai"])
    ...
    output = {
        "summary": summarize(rows, args.label, args.backend),
        "rows": rows,
    }
